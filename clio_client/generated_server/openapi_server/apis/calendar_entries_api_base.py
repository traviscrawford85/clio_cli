...
class BaseCalendarEntriesApi:
    ...
    async def calendar_entry_index(
        self,
        x_api_version: Annotated[Optional[StrictStr], Field(description="The [API minor version](#section/Minor-Versions). Default: latest version.")],
        calendar_id: Annotated[Optional[StrictInt], Field(description="The unique identifier for a single Calendar. The keyword `null` is not valid for this field. The list will be filtered to include only the CalendarEntry records with the matching property.")],
        created_since: Annotated[Optional[datetime], Field(description="Filter CalendarEntry records to those having the `created_at` field after a specific time. (Expects an ISO-8601 timestamp).")],
        expanded: Annotated[Optional[StrictBool], Field(description="Returns a record for each occurrence of a recurring calendar event.  Non-recurring calendar events are unaffected and returned as separate records regardless of the expanded setting.")],
        ...
        fields: Annotated[Optional[StrictStr], Field(description="The fields to be returned. See response samples for what fields are available. For more information see the [fields section](#section/Fields).")],
        var_from: Annotated[Optional[datetime], Field(description="Filter CalendarEntry records to those that end on or after the provided time (Expects an ISO-8601 timestamp).")],
        has_court_rule: Annotated[Optional[StrictBool], Field(description="Allows matching court rule on calendar entry.")],
        ids: Annotated[Optional[StrictInt], Field(description="Filter CalendarEntry records to those having the specified unique identifiers.")],
        is_all_day: Annotated[Optional[StrictBool], Field(description="Filter CalendarEntry records to those that are marked as all day events.")],
        limit: Annotated[Optional[StrictInt], Field(description="A limit on the number of CalendarEntry records to be returned. Limit can range between 1 and 200. Default: `200`.")],
        matter_id: Annotated[Optional[StrictInt], Field(description="The unique identifier for a single Matter. Use the keyword `null` to match those without a CalendarEntry. The list will be filtered to include only the CalendarEntry records with the matching property.")],
        owner_entries_across_all_users: Annotated[Optional[StrictBool], Field(description="Returns CalendarEntry records for all users related to a matter. Requires matter id.")],
        page_token: Annotated[Optional[StrictStr], Field(description="A token specifying which page to return.")],
        query: Annotated[Optional[StrictStr], Field(description="Allows matching search on calendar entry.")],
        source: Annotated[Optional[StrictStr], Field(description="Filter CalendarEntry records to those having a specific calendar visibility source (mobile, web).")],
        to: Annotated[Optional[datetime], Field(description="Filter CalendarEntry records to those that begin on or before the provided time (Expects an ISO-8601 timestamp).")],
        updated_since: Annotated[Optional[datetime], Field(description="Filter CalendarEntry records to those having the `updated_at` field after a specific time. (Expects an ISO-8601 timestamp).")],
        visible: Annotated[Optional[StrictBool], Field(description="Filter CalendarEntry records to those that are visible.")],
    ) -> CalendarEntryList:
