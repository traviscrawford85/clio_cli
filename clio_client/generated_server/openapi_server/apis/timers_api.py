...
from openapi_server.apis.timers_api_base import BaseTimersApi
...
@router.post(
    "/timer.json",
    responses={
        201: {"model": TimerShow, "description": "Created"},
...
async def timer_create(
    ...
    return await BaseTimersApi.subclasses[0]().timer_create(x_api_version, fields, timer_create_request)
...
async def timer_destroy(
    ...
    if not BaseTimersApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseTimersApi.subclasses[0]().timer_destroy(x_api_version)


@router.get(
    "/timer.json",
    responses={
        200: {"model": TimerShow, "description": "Ok"},
        400: {"model": Error, "description": "Bad Request"},
        403: {"model": Error, "description": "Forbidden"},
        404: {"model": Error, "description": "Not Found"},
        401: {"model": Error, "description": "Unauthorized"},
...
async def timer_show(
    ...
    if not BaseTimersApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseTimersApi.subclasses[0]().timer_show(if_modified_since, if_none_match, x_api_version, fields)
