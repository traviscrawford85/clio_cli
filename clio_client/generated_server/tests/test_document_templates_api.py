# coding: utf-8

from fastapi.testclient import TestClient


from datetime import date, datetime  # noqa: F401
from pydantic import Field, StrictInt, StrictStr, field_validator  # noqa: F401
from typing import Any, Optional  # noqa: F401
from typing_extensions import Annotated  # noqa: F401
from openapi_server.models.document_template_create_request import DocumentTemplateCreateRequest  # noqa: F401
from openapi_server.models.document_template_list import DocumentTemplateList  # noqa: F401
from openapi_server.models.document_template_show import DocumentTemplateShow  # noqa: F401
from openapi_server.models.document_template_update_request import DocumentTemplateUpdateRequest  # noqa: F401
from openapi_server.models.error import Error  # noqa: F401


def test_document_template_create(client: TestClient):
    """Test case for document_template_create

    Create a new DocumentTemplate
    """
    document_template_create_request = openapi_server.DocumentTemplateCreateRequest()
    params = [("fields", 'fields_example')]
    headers = {
        "x_api_version": 'x_api_version_example',
    }
    # uncomment below to make a request
    #response = client.request(
    #    "POST",
    #    "/document_templates.json",
    #    headers=headers,
    #    json=document_template_create_request,
    #    params=params,
    #)

    # uncomment below to assert the status code of the HTTP response
    #assert response.status_code == 200


def test_document_template_destroy(client: TestClient):
    """Test case for document_template_destroy

    Delete a single DocumentTemplate
    """

    headers = {
        "x_api_version": 'x_api_version_example',
    }
    # uncomment below to make a request
    #response = client.request(
    #    "DELETE",
    #    "/document_templates/{id}.json".format(id=56),
    #    headers=headers,
    #)

    # uncomment below to assert the status code of the HTTP response
    #assert response.status_code == 200


def test_document_template_download(client: TestClient):
    """Test case for document_template_download

    Get the contents of the DocumentTemplate
    """

    headers = {
    }
    # uncomment below to make a request
    #response = client.request(
    #    "GET",
    #    "/document_templates/{id}/download.json".format(id=56),
    #    headers=headers,
    #)

    # uncomment below to assert the status code of the HTTP response
    #assert response.status_code == 200


def test_document_template_index(client: TestClient):
    """Test case for document_template_index

    Return the data for all DocumentTemplates
    """
    params = [("created_since", '2013-10-20T19:20:30+01:00'),     ("fields", 'fields_example'),     ("ids", 56),     ("limit", 56),     ("order", 'order_example'),     ("page_token", 'page_token_example'),     ("updated_since", '2013-10-20T19:20:30+01:00')]
    headers = {
        "x_api_version": 'x_api_version_example',
    }
    # uncomment below to make a request
    #response = client.request(
    #    "GET",
    #    "/document_templates.json",
    #    headers=headers,
    #    params=params,
    #)

    # uncomment below to assert the status code of the HTTP response
    #assert response.status_code == 200


def test_document_template_show(client: TestClient):
    """Test case for document_template_show

    Return the data for a single DocumentTemplate
    """
    params = [("fields", 'fields_example')]
    headers = {
        "if_modified_since": '2013-10-20',
        "if_none_match": 'if_none_match_example',
        "x_api_version": 'x_api_version_example',
    }
    # uncomment below to make a request
    #response = client.request(
    #    "GET",
    #    "/document_templates/{id}.json".format(id=56),
    #    headers=headers,
    #    params=params,
    #)

    # uncomment below to assert the status code of the HTTP response
    #assert response.status_code == 200


def test_document_template_update(client: TestClient):
    """Test case for document_template_update

    Update a single DocumentTemplate
    """
    document_template_update_request = openapi_server.DocumentTemplateUpdateRequest()
    params = [("fields", 'fields_example')]
    headers = {
        "if_match": 'if_match_example',
        "x_api_version": 'x_api_version_example',
    }
    # uncomment below to make a request
    #response = client.request(
    #    "PATCH",
    #    "/document_templates/{id}.json".format(id=56),
    #    headers=headers,
    #    json=document_template_update_request,
    #    params=params,
    #)

    # uncomment below to assert the status code of the HTTP response
    #assert response.status_code == 200

