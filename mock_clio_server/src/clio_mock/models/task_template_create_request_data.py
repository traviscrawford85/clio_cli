...
from clio_mock.models.task_template_create_request_data_cascading_source import TaskTemplateCreateRequestDataCascadingSource
from clio_mock.models.task_template_create_request_data_reminder_templates_inner import TaskTemplateCreateRequestDataReminderTemplatesInner
from clio_mock.models.task_template_create_request_data_task_template_list import TaskTemplateCreateRequestDataTaskTemplateList
try:
...
class TaskTemplateCreateRequestData(BaseModel):
    ...
    description: Optional[StrictStr] = Field(default=None, description="Longer description of the TaskTemplate. This TaskTemplate supports rich text when the `description_text_type` field is set to `rich_text`. With supported tags such as `<a>`, `<b>`, `<br>`, `<div>`, `<em>`, `<i>` `<li>`, `<ol>`, `<p>`, `<s>`, `<strong>`, `<u>` and `<ul>`. This TaskTemplate also supports attributes such as `href`, `rel`, `type`, and `target`.")
    description_text_type: Optional[StrictStr] = Field(default='plain_text', description="The type of text in the description field.")
    ...
    __properties: ClassVar[List[str]] = ["cascading", "cascading_offset", "cascading_offset_polarity", "cascading_offset_type", "cascading_source", "description", "description_text_type", "name", "priority", "private", "reminder_templates", "task_template_list"]
    ...
    @field_validator('description_text_type')
    def description_text_type_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in ('plain_text', 'rich_text',):
            raise ValueError("must be one of enum values ('plain_text', 'rich_text')")
        return value
    ...
    def from_dict(cls, obj: Dict) -> Self:
        ...
        _obj = cls.model_validate({
            ...
            "description_text_type": obj.get("description_text_type") if obj.get("description_text_type") is not None else 'plain_text',
            "name": obj.get("name"),
