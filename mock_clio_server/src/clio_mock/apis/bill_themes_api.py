...
from clio_mock.apis.bill_themes_api_base import BaseBillThemesApi
...
from clio_mock.models.bill_theme_update_request import BillThemeUpdateRequest
...
async def bill_theme_index(
    ...
    return await BaseBillThemesApi.subclasses[0]().bill_theme_index(x_api_versio_nheader, created_sincequery, fieldsquery, idsquery, limitquery, page_tokenquery, updated_sincequery)
...
async def bill_theme_update(
    ...
    if not BaseBillThemesApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseBillThemesApi.subclasses[0]().bill_theme_update(idpath, if_matc_hheader, x_api_versio_nheader, fieldsquery, bill_theme_update_request)
