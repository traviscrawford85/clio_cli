...
from clio_mock.apis.notes_api_base import BaseNotesApi
...
async def note_create(
    ...
    return await BaseNotesApi.subclasses[0]().note_create(x_api_versio_nheader, fieldsquery, note_create_request)
...
async def note_destroy(
    ...
    if not BaseNotesApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseNotesApi.subclasses[0]().note_destroy(idpath, x_api_versio_nheader)
...
async def note_index(
    ...
    fieldsquery: Annotated[Optional[StrictStr], Field(description="The fields to be returned. See response samples for what fields are available. For more information see the [fields section](#section/Fields).")] = Query(None, description="The fields to be returned. See response samples for what fields are available. For more information see the [fields section](#section/Fields).", alias="fieldsquery"),
    has_time_entriesquery: Annotated[Optional[StrictBool], Field(description="Filter Note records to those with or without associated time entries.")] = Query(None, description="Filter Note records to those with or without associated time entries.", alias="has_time_entriesquery"),
    ...
    if not BaseNotesApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseNotesApi.subclasses[0]().note_index(x_api_versio_nheader, contact_idquery, created_sincequery, fieldsquery, has_time_entriesquery, idsquery, limitquery, matter_idquery, orderquery, page_tokenquery, queryquery, typequery, updated_sincequery)
...
async def note_show(
    ...
    if not BaseNotesApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseNotesApi.subclasses[0]().note_show(idpath, if_modified_sinc_eheader, if_none_matc_hheader, x_api_versio_nheader, fieldsquery)
...
async def note_update(
    ...
    if not BaseNotesApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseNotesApi.subclasses[0]().note_update(idpath, if_matc_hheader, x_api_versio_nheader, fieldsquery, note_update_request)
