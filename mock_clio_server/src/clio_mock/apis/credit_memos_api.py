...
from clio_mock.apis.credit_memos_api_base import BaseCreditMemosApi
...
async def credit_memo_index(
    ...
    return await BaseCreditMemosApi.subclasses[0]().credit_memo_index(x_api_versio_nheader, contact_idquery, created_sincequery, fieldsquery, idsquery, limitquery, orderquery, page_tokenquery, updated_sincequery)
...
async def credit_memo_show(
    ...
    if not BaseCreditMemosApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseCreditMemosApi.subclasses[0]().credit_memo_show(idpath, if_modified_sinc_eheader, if_none_matc_hheader, x_api_versio_nheader, fieldsquery)
