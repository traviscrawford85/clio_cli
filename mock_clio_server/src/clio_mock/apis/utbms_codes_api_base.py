...
class BaseUtbmsCodesApi:
    ...
    async def utbms_code_index(
        ...
        utbms_set_idquery: Annotated[Optional[StrictInt], Field(description="The unique identifier for a single UtbmsSet. Use the keyword `null` to match those without a UtbmsCode. The list will be filtered to include only the UtbmsCode records with the matching property.")],
    ) -> ActivityList:
