...
from clio_mock.apis.allocations_api_base import BaseAllocationsApi
...
async def allocation_index(
    ...
    return await BaseAllocationsApi.subclasses[0]().allocation_index(x_api_versio_nheader, bill_idquery, contact_idquery, created_sincequery, fieldsquery, idsquery, limitquery, matter_idquery, orderquery, page_tokenquery, parent_idquery, parent_typequery, statusquery, updated_sincequery)
...
async def allocation_show(
    ...
    if not BaseAllocationsApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseAllocationsApi.subclasses[0]().allocation_show(idpath, if_modified_sinc_eheader, if_none_matc_hheader, x_api_versio_nheader, fieldsquery)
