...
from clio_mock.apis.service_types_api_base import BaseServiceTypesApi
...
async def service_type_index(
    ...
    return await BaseServiceTypesApi.subclasses[0]().service_type_index(x_api_versio_nheader, created_sincequery, fieldsquery, idsquery, limitquery, orderquery, page_tokenquery, updated_sincequery)
...
async def service_type_show(
    ...
    if not BaseServiceTypesApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseServiceTypesApi.subclasses[0]().service_type_show(idpath, if_modified_sinc_eheader, if_none_matc_hheader, x_api_versio_nheader, fieldsquery)
