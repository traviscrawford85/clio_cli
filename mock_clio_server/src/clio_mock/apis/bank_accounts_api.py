...
from clio_mock.apis.bank_accounts_api_base import BaseBankAccountsApi
...
async def bank_account_create(
    ...
    return await BaseBankAccountsApi.subclasses[0]().bank_account_create(x_api_versio_nheader, fieldsquery, bank_account_create_request)
...
async def bank_account_destroy(
    ...
    if not BaseBankAccountsApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseBankAccountsApi.subclasses[0]().bank_account_destroy(idpath, x_api_versio_nheader)
...
async def bank_account_index(
    ...
    show_empty_accountsquery: Annotated[Optional[StrictBool], Field(description="Filter BankAccount records to those having a zero or non zero balance.")] = Query(None, description="Filter BankAccount records to those having a zero or non zero balance.", alias="show_empty_accountsquery"),
    ...
    if not BaseBankAccountsApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseBankAccountsApi.subclasses[0]().bank_account_index(x_api_versio_nheader, created_sincequery, fieldsquery, idsquery, limitquery, orderquery, page_tokenquery, show_empty_accountsquery, typequery, updated_sincequery, user_idquery)
...
async def bank_account_show(
    ...
    if not BaseBankAccountsApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseBankAccountsApi.subclasses[0]().bank_account_show(idpath, if_modified_sinc_eheader, if_none_matc_hheader, x_api_versio_nheader, fieldsquery)
...
async def bank_account_update(
    ...
    if not BaseBankAccountsApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseBankAccountsApi.subclasses[0]().bank_account_update(idpath, if_matc_hheader, x_api_versio_nheader, fieldsquery, bank_account_update_request)
