...
from clio_mock.apis.conversation_messages_api_base import BaseConversationMessagesApi
...
async def conversation_message_create(
    ...
    return await BaseConversationMessagesApi.subclasses[0]().conversation_message_create(x_api_versio_nheader, fieldsquery, conversation_message_create_request)
...
async def conversation_message_index(
    conversation_idquery: Annotated[StrictInt, Field(description="The unique identifier for a single Conversation. Use the keyword `null` to match those without a ConversationMessage. The list will be filtered to include only the ConversationMessage records with the matching property.")] = Query(None, description="The unique identifier for a single Conversation. Use the keyword &#x60;null&#x60; to match those without a ConversationMessage. The list will be filtered to include only the ConversationMessage records with the matching property.", alias="conversation_idquery"),
    x_api_versio_nheader: Annotated[Optional[StrictStr], Field(description="The [API minor version](#section/Minor-Versions). Default: latest version.")] = Header(None, description="The [API minor version](#section/Minor-Versions). Default: latest version."),
    ...
    if not BaseConversationMessagesApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseConversationMessagesApi.subclasses[0]().conversation_message_index(conversation_idquery, x_api_versio_nheader, created_sincequery, fieldsquery, idsquery, limitquery, orderquery, page_tokenquery, queryquery, updated_sincequery)
...
async def conversation_message_show(
    ...
    if not BaseConversationMessagesApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseConversationMessagesApi.subclasses[0]().conversation_message_show(idpath, if_modified_sinc_eheader, if_none_matc_hheader, x_api_versio_nheader, fieldsquery)
