...
from clio_mock.apis.jurisdictions_api_base import BaseJurisdictionsApi
...
async def jurisdiction_index(
    ...
    for_current_accountquery: Annotated[Optional[StrictBool], Field(description="Filter Jurisdiction records to those set up for this account.")] = Query(None, description="Filter Jurisdiction records to those set up for this account.", alias="for_current_accountquery"),
    ...
    return await BaseJurisdictionsApi.subclasses[0]().jurisdiction_index(x_api_versio_nheader, created_sincequery, fieldsquery, for_current_accountquery, idsquery, limitquery, orderquery, page_tokenquery, queryquery, updated_sincequery)
...
async def jurisdiction_show(
    ...
    if not BaseJurisdictionsApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseJurisdictionsApi.subclasses[0]().jurisdiction_show(idpath, if_modified_sinc_eheader, if_none_matc_hheader, x_api_versio_nheader, fieldsquery)
