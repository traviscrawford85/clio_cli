...
from clio_mock.apis.users_api_base import BaseUsersApi
...
async def user_index(
    ...
    enabledquery: Annotated[Optional[StrictBool], Field(description="Filter TaskType records to those that are enabled or disabled.")] = Query(None, description="Filter TaskType records to those that are enabled or disabled.", alias="enabledquery"),
    ...
    include_co_counselquery: Annotated[Optional[StrictBool], Field(description="Filter User to include co-counsel users")] = Query(None, description="Filter User to include co-counsel users", alias="include_co_counselquery"),
    ...
    pending_setupquery: Annotated[Optional[StrictBool], Field(description="Filter User records based on whether or not they are still being setup.")] = Query(None, description="Filter User records based on whether or not they are still being setup.", alias="pending_setupquery"),
    rolequery: Annotated[Optional[StrictStr], Field(description="Filter User records to those with a specific role.")] = Query(None, description="Filter User records to those with a specific role.", alias="rolequery"),
    subscription_typequery: Annotated[Optional[StrictStr], Field(description="Filter User records to those with a specific subscription type.")] = Query(None, description="Filter User records to those with a specific subscription type.", alias="subscription_typequery"),
    ...
    return await BaseUsersApi.subclasses[0]().user_index(x_api_versio_nheader, created_sincequery, enabledquery, fieldsquery, idsquery, include_co_counselquery, limitquery, namequery, orderquery, page_tokenquery, pending_setupquery, rolequery, subscription_typequery, updated_sincequery)
...
async def user_show(
    ...
    if not BaseUsersApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseUsersApi.subclasses[0]().user_show(idpath, if_modified_sinc_eheader, if_none_matc_hheader, x_api_versio_nheader, fieldsquery)
...
async def user_who_am_i(
    ...
    if not BaseUsersApi.subclasses:
        raise HTTPException(status_code=500, detail="Not implemented")
    return await BaseUsersApi.subclasses[0]().user_who_am_i(if_modified_sinc_eheader, if_none_matc_hheader, x_api_versio_nheader, fieldsquery)
